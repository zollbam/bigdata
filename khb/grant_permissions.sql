          ...
          FROM (
                SELECT
                 ...
                 WHERE class_desc != 'DATABASE' 
                   ...
                   AND object_name(major_id) = ANY(SELECT name 
                                                     FROM sys.tables 
                                                    WHERE schema_id = 5)
               ) AS a
                    PIVOT (
...
SELECT 
	  b."객체명"
	, b."권한 받은 유저명"
	, CASE WHEN b."UPDATE" = 1 THEN 'O'
	       ELSE 'X'
	  END "update"
	, CASE WHEN b."ALTER" = 1 THEN 'O'
	       ELSE 'X'
	  END "alter"
  FROM (
		SELECT * 
		  FROM (
				SELECT
					  object_name(major_id) "객체명"
					, user_name(grantee_principal_id) "권한 받은 유저명"
					, permission_name "권한명"
					, 'O' res
				  FROM sys.DATABASE_permissions dp
				 WHERE class_desc != 'DATABASE' 
				   AND grantee_principal_id != 0
				   AND object_name(major_id) = ANY(SELECT name FROM sys.sequences)
			   ) AS a
			        PIVOT (
			               count(a.res) FOR "권한명" IN ([UPDATE], [ALTER])
			              ) AS pivot_res
	   ) AS b;
