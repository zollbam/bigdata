...
-- 인덱스
/*
- postgresql은 기본키나 unique 제약조건을 추가할 때마다 제약조건에 포함된 열에 인덱스를 생성되고
  create index문을 실행할 때 기본적으로 생성되는 유형이기도 함
- 인덱스는 테이블 데이터와 별도로 저장되고 쿼리를 실행할 때 자동으로 액세스되고
  행이 추가나 제거, 업데이트 될 때 마다 업데이트 됩니다.
- 
*/

-- * 예제
/*
검색 쿼리 속도를 높이기 위해 postgresql의 기본형인 b-tree인덱스를 사용
이 데이터는 940374행 * 7열의 뉴욕 주소로 구성되었다.
*/
...
-- * EXPLAIN로 쿼리 성능 비교
/*
인덱스를 추가하기 전후의 성능을 측정하기 위해 postgresql에서만 지원하는 explain명령을 사용
analyze키워드를 추가하면 explain이 쿼리를 수행하고 실제 시간을 표시 

하드웨어 성능, 다른 프로세스, 앞선 쿼리의 결과로 인한 메모리 유지 등 다양한 이유로 실행 시간이 조금씩 다르다.
*/
...
/*
new_york_address테이블의 street열을 기준으로 인덱스를 생성하였습니다.
문법: CREATE INDEX [인덱스명] ON [테이블명(컬럼명, 컬럼명, ...)]
*/
